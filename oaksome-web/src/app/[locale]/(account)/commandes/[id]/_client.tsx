...
function getRdvCTA(project: ProjectDetail, t: ReturnType<typeof useTranslations>): { show: boolean; label: string; sub: string } {
  ...
  if (so2Status === 'manufacturing' || so2Status === 'ready' || so2Status === 'delivering') {
...
function formatPrice(amount: number, _currency: string, locale: string): string {
...
function formatAmount(amount: number, _currency: string, locale: string): string {
  return new Intl.NumberFormat(toDateLocale(locale), {
    style: 'currency', currency: 'EUR',
...
function ProjetDetailPageContent() {
  ...
  useEffect(() => {
    ...
    getProjectDetail(projectId).then(result => {
      if (result.success) {
        ...
        if ((so2Status === 'manufacturing' || so2Status === 'ready' || so2Status === 'delivering') && result.data.so1_id) {
  ...
  if (error || !project) return (
    <div className="orders-empty">
      ...
      <Link href="/commandes" className="order-detail-back">{t('account.projectDetail.back')}</Link>
    </div>
  )
  ...
  return (
    <div>
      <Link href="/commandes" className="order-detail-back">
      ...
      {rdvCTA.show && (() => {
        ...
        return (
          <div className="rdv-cta-banner">
            ...
            <Link href={{ pathname: '/commandes/[id]/rendez-vous', params: { id: params.id } }} className="rdv-cta-link">
      ...
      <div className="detail-card">
        ...
        {(() => {
          ...
          const deductionsTotal = (ps?.deductions ?? []).reduce((s, d) => s + d.amount, 0)
          // Deductions only affect the SO1 display (SO2 total is already final, no deduction to add back)
          const isSo2 = !!project.so2
          const grossTtc = (!isSo2 && deductionsTotal > 0) ? totalTtc + deductionsTotal : totalTtc
          // 1er acompte: formula only for SO1-only view; on SO2 the invoice amount is authoritative
          const so1DepositDisplay = (!isSo2 && deductionsTotal > 0)
            ? totalTtc * 0.5
            : (ps?.so1_deposit?.amount ?? totalTtc * 0.5)
          return (
            <>
              {/* Total projet */}
              <div className="summary-row total" style={{ marginBottom: (!isSo2 && deductionsTotal > 0) ? '0.5rem' : '1rem' }}>
                <span>{project.so2 ? t('account.projectDetail.total_ttc_final') : t('account.projectDetail.total_ttc_initial')}</span>
                <span>{formatAmount(grossTtc, cur, locale)}</span>
              </div>

              {/* Déductions showroom mobile / kit — uniquement sur la vue SO1 (avant devis définitif) */}
              {!isSo2 && ps?.deductions && ps.deductions.length > 0 && (
                <>
                  {ps.deductions.map((d, i) => (
                    <div key={i} className="summary-row" style={{ fontSize: '0.9rem' }}>
                      <span style={{ color: '#696761' }}>
                        {d.type === 'showroom_mobile'
                          ? t('account.projectDetail.deduction_showroom')
                          : t('account.projectDetail.deduction_kit')}
                      </span>
                      <span style={{ color: '#0C524E', fontWeight: 600 }}>−{formatAmount(d.amount, cur, locale)}</span>
                    </div>
                  ))}
                  <div className="summary-row" style={{ fontSize: '0.9rem', marginBottom: '1rem' }}>
                    <span style={{ color: '#696761' }}>{t('account.projectDetail.total_after_deductions')}</span>
                    <span style={{ fontWeight: 600 }}>{formatAmount(totalTtc, cur, locale)}</span>
                  </div>
                </>
              )}
              ...
              <div className="summary-row">
                ...
                <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.1rem' }}>
                  <span>{formatAmount(so1DepositDisplay, cur, locale)}</span>
              ...
              <div className="summary-row">
                <span style={{ display: 'flex', flexDirection: 'column', gap: '0.1rem' }}>
                  ...
                  {ps?.balance_paid
                    ? ps.balance_date && <span style={{ fontSize: '0.78rem', color: '#696761' }}>{formatDate(ps.balance_date, locale)}</span>
                ...
                <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.1rem' }}>
                  {ps?.balance != null ? (
                    <>
                      ...
                      <span style={{ fontSize: '0.75rem', fontWeight: 600, color: ps.balance_paid ? '#0C524E' : '#696761' }}>
                        {ps.balance_paid
                          ? t('account.projectDetail.paid')
                          : t('account.projectDetail.after_installation')}
                      </span>
                    </>
                  ) : (
