...
import { getSo2Balance } from '@/lib/api/payment'
import type { So2Balance } from '@/lib/api/payment'
...
export default function BookingPage() {
  ...
  const [so2Id,        setSo2Id]        = useState<number | null>(null)
  const [orderName,    setOrderName]    = useState<string>('')
  const [orderStatus,  setOrderStatus]  = useState<string>('')
  const [so2Status,    setSo2Status]    = useState<string | null>(null)
  const [so2Balance,   setSo2Balance]   = useState<So2Balance | null>(null)
  const [so2DateOrder, setSo2DateOrder] = useState<string | null>(null)
  ...
  useEffect(() => {
    getProjectDetail(projectId).then(r => {
      if (r.success) {
        ...
        const fetchedSo2Id = r.data.so2?.id ?? null
        if (fetchedSo2Id) {
          setSo2Id(fetchedSo2Id)
          if (r.data.so2?.date) setSo2DateOrder(r.data.so2.date.split('T')[0])
          getSo2Balance(fetchedSo2Id).then(br => { if (br.success) setSo2Balance(br.data) })
        }
        ...
        // Installation : SO2 phase (manufacturing, ready, delivering)
        } else if (fetchedSo2Status === 'manufacturing' || fetchedSo2Status === 'ready' || fetchedSo2Status === 'delivering') {
  ...
  // Minimum date for installation: SO2 date + pose_delay_days + 1
  // date_order: from so2Balance (API) or so2DateOrder (project detail, available sooner)
  const minInstallDate = (() => {
    if (selectedType !== 'installation') return null
    const dateStr = so2Balance?.date_order ?? so2DateOrder
    if (!dateStr) return null
    const delay = so2Balance?.pose_delay_days ?? null
    if (delay === null) return null   // pose_delay_days not yet loaded — wait before blocking
    const base = new Date(dateStr)
    base.setDate(base.getDate() + delay + 1)
    return base
  })()
  ...
  function isDayBeforeMinDate(y: number, m: number, d: number): boolean {
    if (!minInstallDate) return false
    return new Date(y, m, d) < minInstallDate
  }

  const daysInMonth    = getDaysInMonth(calYear, calMonth)
  const firstOffset    = getFirstDayOffset(calYear, calMonth)
  const minDateY = minInstallDate ? minInstallDate.getFullYear() : TODAY_Y
  const minDateM = minInstallDate ? minInstallDate.getMonth() : TODAY_M
  const isPrevDisabled = calYear === Math.min(TODAY_Y, minDateY) && calMonth === Math.min(TODAY_M, minDateM) && calYear <= TODAY_Y
  ...
  function selectDay(day: number) {
    ...
    const unavail = isUnavailable(calYear, calMonth, day)
      || isDayBeforeMinDate(calYear, calMonth, day)
      || (!slotsLoading && !availableDays.has(key))
    if (unavail) return
  ...
  async function confirm() {
    ...
    } else if (result.code === 402 && selectedType === 'installation' && so2Id) {
      // Final 10% payment required — redirect to checkout with slot data so it auto-books after payment
      router.push({
        pathname: '/checkout',
        query: {
          final:      '1',
  ...
  const visibleCards = typeCards.filter(card => {
    ...
    if (so2Status === 'manufacturing' || so2Status === 'ready' || so2Status === 'delivering') return card.id === 'installation'
    return false
  })
  ...
  if (confirmed && selectedDay !== null && selectedTime !== null) {
    return (
      <div className="booking-confirmed">
        ...
        <Link href={`/commandes/${params?.id ?? ''}`} className="booking-confirmed-back">
          {t('back_project')}
        </Link>
      </div>
    )
  ...
  if (orderStatus === 'measures_scheduled' && existingAppointment && !isModifying) {
    ...
    return (
      <div>
        <Link href={`/commandes/${params?.id ?? ''}`} className="booking-back">
          {t('back_to_project')}
        </Link>
          ...
          <div className="cal-wrap">
            ...
            <div className="cal-grid">
              ...
              {!slotsLoading && Array.from({ length: daysInMonth }, (_, i) => {
                ...
                const unavail = pastOrWeekend || isDayBeforeMinDate(calYear, calMonth, day) || !availableDays.has(key)
