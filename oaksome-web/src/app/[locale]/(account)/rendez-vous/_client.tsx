...
function RdvCard({ rdv, past, cancelling, onRequestCancel }: RdvCardProps) {
  ...
  function handleModify() {
    if (rdv.order_id) {
      router.push({ pathname: '/commandes/[id]/rendez-vous', params: { id: String(rdv.order_id) } })
    } else {
      ...
      <div className="rdv-card-info">
        ...
        {rdv.order_name && (
          <p className="rdv-card-order">
            {t('card_project_prefix')}<Link href={{ pathname: '/commandes/[id]', params: { id: String(rdv.order_id) } }} style={{ color: '#0C524E', textDecoration: 'none', fontWeight: 600 }}>{rdv.order_name}</Link>
          </p>
        )}
      </div>
      ...
      <div className="profile-section">
        ...
        {loading ? (
          ...
          <div className="rdv-empty">
            ...
            <Link href="/commandes" className="btn btn-primary rdv-cta">
              {t('see_projects')}
            </Link>
          </div>
        ) : (
