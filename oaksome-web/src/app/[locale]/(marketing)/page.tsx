...
import { CollectionsScroll } from './_components/collections-scroll'
import { TestimonialsSlider } from './_components/testimonials-slider'
import { HeroCombo } from './_components/hero-combo'
...
export default async function HomePage({ params }: Props) {
  ...
  const [navResult, inspoResult, homeResult, comboResult] = await Promise.all([
    getNavigation(locale),
    getHomepageInspirations(locale),
    getHomeData(locale),
    getComboConfig(locale),
  ])
  // Defend against partial/degraded Odoo responses: a `success` result whose
  // nested array field is missing (observed while Odoo was mid module-upgrade)
  // must degrade to an empty section, not crash the whole page via `.length`/`.map`.
  ...
  const comboConfig = comboResult.success && comboResult.data && (comboResult.data.banners?.length ?? 0) > 0
    ? comboResult.data
    : null
        ...
        <div className="container">
          ...
          <div className="re-grid">
            {[
              {
                anchor: 'online',
                img: '/images/reassurance-online.png',
                alt: t('home.reassurance.online_alt'),
                tag: t('home.reassurance.online_tag'),
                title: t('home.reassurance.online_title'),
                desc: t('home.reassurance.online_desc')
              },
              {
                anchor: 'samples',
                img: '/images/reassurance-samples.png',
                alt: t('home.reassurance.samples_alt'),
                tag: t('home.reassurance.samples_tag'),
                tagAccent: true,
                title: t('home.reassurance.samples_title'),
                desc: t('home.reassurance.samples_desc'),
                highlight: true
              },
              {
                anchor: 'kit',
                img: '/images/reassurance-kit.png',
                alt: t('home.reassurance.kit_alt'),
                tag: t('home.reassurance.kit_tag'),
                title: t('home.reassurance.kit_title'),
                desc: t('home.reassurance.kit_desc')
              },
              {
                anchor: 'showroom',
                img: '/images/reassurance-showroom.png',
                alt: t('home.reassurance.showroom_alt'),
                tag: t('home.reassurance.showroom_tag'),
                title: t('home.reassurance.showroom_title'),
                desc: t('home.reassurance.showroom_desc')
              },
            ].map((card, i) => (
                <Link key={i} href={`/echantillons#${card.anchor}`}
                      className={`re-card${card.highlight ? ' re-card--highlight' : ''}`}>
                  <div className="re-card-photo">
                    <img src={card.img} alt={card.alt}  style={{objectFit: 'cover'}} loading="eager"/>
                  </div>
                  ...
                  <span className="re-cta">
            {t('home.reassurance.discover_cta')} <span className="re-cta-arrow">→</span>
          </span>
                </Link>
            ))}
          </div>
                      ...
                      <div style={{ marginTop: '12px' }}>
                        <span className="cta-discover-double">
                          <Link href="/configurer" className="cta-action">
                            {t('home.how_works.s1_cta_configure')} <span className="cta-arrow">→</span>
                          </Link>
                          <Link href="/echantillons" className="cta-label">
                            {t('home.how_works.s1_cta_samples')}
                          </Link>
                        </span>
                      </div>
                    </>
