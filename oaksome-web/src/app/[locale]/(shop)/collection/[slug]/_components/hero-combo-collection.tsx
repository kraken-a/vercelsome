...
type StyleInfo = { name: string; slug: string }
type FilterKey = 'space' | 'category'
type Filters = { spaces: Set<number>; categories: Set<number> }
...
export function HeroComboCollection({ config, style, allStyles = [] }: Props) {
  const t = useTranslations('home')
  const { banners } = config

  const [filters, setFilters] = useState<Filters>({ spaces: new Set(), categories: new Set() })
  ...
  const styles = allStyles.length > 0 ? allStyles : (() => {
    const seen = new Set<string>()
    return banners.filter(b => b.style).map(b => b.style!).filter(s => {
  ...
  const toggleFilter = useCallback((type: 'spaces' | 'categories', id: number) => {
    setFilters(prev => {
  ...
  const closeAll = () => { setOpenDropdowns(new Set()); setStyleOpen(false) }
  ...
  return (
    <section className="hero-combo hero-combo--collection" onClick={closeAll} aria-label={style.name}>
      <h1 className="sr-only">{style.name}</h1>
      ...
      <div className="hh-controls">
        <div className={`hh-selectors${mobileExpanded ? ' is-mobile-open' : ''}`}>
          ...
          {/* Style pill — dropdown navigation vers autres collections */}
          <div className="hh-pill-wrap hh-pill-wrap--filter">
            {styleOpen && styles.length > 1 && (
              <div className="hh-pop hh-pop--offset" role="dialog" onClick={e => e.stopPropagation()}>
                <div className="hh-pop-head">
                  <span className="hh-pop-label">COLLECTION</span>
                  <button type="button" className="hh-pop-x" onClick={() => setStyleOpen(false)} aria-label="Fermer">×</button>
                </div>
                <ul className="hh-pop-list">
                  {styles.map(s => (
                    <li key={s.slug}>
                      <Link
                        ...
                        className={`hh-pop-opt${s.slug === style.slug ? ' is-active' : ''}`}
            ...
            <button
              type="button"
              className="hh-sel hh-sel--lime"
              onClick={(e) => { e.stopPropagation(); setStyleOpen(v => !v) }}
              aria-expanded={styleOpen}
              aria-haspopup="dialog"
            >
              <span className="hh-dot" />
              <span className="hh-lbl">{style.name}</span>
              <GridIcon />
            </button>
          </div>
          ...
          {/* Configurator CTA */}
          <Link href={`/configurer?collection=${style.slug}`} className="hh-configurer" aria-label={t('hero.cta')}>
