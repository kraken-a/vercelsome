...
// Default-deny allowlist of cookies forwarded upstream to Odoo (M3).
// Only the Odoo session cookie is forwarded. `odoo_sid` is an accepted input
// name that is renamed to `session_id` before forwarding (configurator iframe).
// `frontend_lang` is intentionally NOT allowlisted: Odoo locale is driven by the
// `?lang=` query param (docs/api-contract.md L58-60), not a cookie.
...
async function proxy(req: NextRequest, path: string): Promise<NextResponse> {
  ...
  // Default-deny: only keep allowlisted cookie names; everything else is dropped.
