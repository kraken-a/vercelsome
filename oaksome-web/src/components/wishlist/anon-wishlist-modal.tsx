...
import { trackGenerateLead } from '@/features/tracking/events'
...
export function AnonWishlistModal({ productId, onCloseAction }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(false)
    if (result.success) {
      hashSha256Lower(email.trim()).then(emailHash => {
        trackGenerateLead(result.data.id, emailHash, productId ?? undefined)
      }).catch(() => {})
