...
export default function ProPage() {
  ...
  const values = [
    { icon: '⚙', headsup: t('v1_headsup'), title: t('v1_title'), desc: t('v1_desc')},
    {icon: '★', headsup: t('v2_headsup'), title: t('v2_title'), desc: t('v2_desc')},
    {icon: '📄', headsup: t('v3_headsup'), title: t('v3_title'), desc: t('v3_desc')},
    {icon: '📑', headsup: t('v4_headsup'), title: t('v4_title'), desc: t('v4_desc')},
  ]
  const steps = [
      ...
      <section>
        <div className="container">
          ...
          <div className="spec-grid spec-grid--4">
            {values.map(v => (
                <div key={v.title} className="spec-card">
                  <div className="spec-card-body">
                    {/*<span className="card-icon">{v.icon}</span>*/}
                    <span className="spec-eyebrow">{v.headsup}</span>
                    <h4 >{v.title}</h4>
                    <p>{v.desc}</p>
                  </div>
                </div>
      ...
      <section>
        <div className="container">
          ...
          <div className="testimonial-placeholder">
            <h3 >{t('tests_h3')}</h3>
            <p>{t('tests_intro')}</p>
            <a href="#inscription" className="btn-charte-solid" style={{ marginTop: '1.5rem' }}>{t('tests_cta')}</a>
          </div>
        </div>
      </section>
          ...
          <div className="pro-form-section">
              ...
              <div style={{ marginTop: '1rem' }}>
                {checklist.map(item => (
                  <div key={item} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', marginBottom: '1.2rem' }}>
                    <div className="pro-benefit-checklist">✓</div>
            ...
            <div className="gap8">
              {status === 'success' ? (
                  ...
                  <form className="form-box gap8" onSubmit={handleSubmit}>
                    {status === 'error' && (
                        <p style={{color: 'red', marginBottom: '1rem'}}>{errorMsg}</p>
                    )}
                    <div className="form-row">
                      <div className="form-group">
                        <label htmlFor="pro-nom">{t('label_name')}</label>
                        <input type="text" id="pro-nom" name="name" placeholder={t('ph_name')} value={form.name}
                               onChange={handleChange}/>
                      </div>
                      <div className="form-group">
                        <label htmlFor="pro-societe">{t('label_company')}</label>
                        <input type="text" id="pro-societe" name="company" placeholder={t('ph_company')}
                               value={form.company}
                               onChange={handleChange}/>
                      </div>
                    </div>
                    <div className="form-row">
                      <div className="form-group">
                        <label htmlFor="pro-email">{t('label_email')}</label>
                        <input type="email" id="pro-email" name="email" placeholder={t('ph_email')} value={form.email}
                               onChange={handleChange}/>
                      </div>
                      <div className="form-group">
                        <label htmlFor="pro-tel">{t('label_phone')}</label>
                        <input type="tel" id="pro-tel" name="phone" placeholder={t('ph_phone')} value={form.phone}
                               onChange={handleChange}/>
                      </div>
                    </div>
                    <div className="form-group form-group--select">
                      <label htmlFor="pro-specialite">{t('label_specialty')}</label>
                      <select id="pro-specialite" name="specialty" value={form.specialty} onChange={handleChange}>
                    ...
                    <div className="form-group">
                      <label htmlFor="pro-message">{t('label_message')}</label>
                      <textarea id="pro-message" name="message" placeholder={t('ph_message')} value={form.message}
                                onChange={handleChange}/>
                    </div>
                    <div className="form-actions">
                      <button type="submit" className="form-send" disabled={status === 'loading'}>
