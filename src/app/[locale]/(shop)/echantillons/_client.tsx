...
import './sample.css'
import { getSlots } from '@/lib/api/appointments'
import type { SlotsResponse } from '@/lib/api/appointments'
...
export default function EchantillonsPage() {
  ...
  const [kdBooked, setKdBooked] = useState(false)
  ...
  const fetchShowroomSlots = useCallback((date: Date) => {
    const monthStr = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`
    setShowroomSlotsLoading(true)
    setShowroomSlots(null)
    getSlots('mobile', monthStr).then(r => {
  ...
  useEffect(() => {
    if (activeTab === 'showroom') fetchShowroomSlots(calDate)
  }, [activeTab, calDate, fetchShowroomSlots])
  ...
  async function esSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const p1 = esPairs[0]
    const p2 = esPairs[1]
    const result = await apiPost<{ request_id: string }>('/kit/request', {
  ...
  async function kdSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    if (!kd.handle) { alert(t('kd_alert_select')); return }
    if (!kitProductId) { setKdError('Produit non disponible. Veuillez réessayer.'); return }
    setKdSubmitting(true)
    setKdError(null)
    const ok = await addItem({
      productId: kitProductId,
      name: 'Kit découverte',
      price: kitProductPrice,
      quantity: 1,
      imageUrl: '',
      config: {
        collection: kd.coll,
    ...
    if (ok) {
      setCartOpen(true)
      setKdBooked(true)
      setKd({ coll: null, door: null, ext: null, int: null, handle: null })
    } else {
      setKdError('Une erreur est survenue. Veuillez réessayer.')
    }
  }
  ...
  async function agendaSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!showroomProductId) { setShowroomError('Produit non disponible. Veuillez réessayer.'); return }
    ...
    const ok = await addItem({
      productId: showroomProductId,
      name: 'Showroom mobile',
      price: showroomProductPrice,
      quantity: 1,
      imageUrl: '',
      config: {
        date: dateKey,
    ...
    if (ok) {
      setCartOpen(true)
      setShowroomBooked(true)
      setSelectedDate(null)
      setSelectedSlot(null)
    } else {
      setShowroomError('Une erreur est survenue. Veuillez réessayer.')
    }
  }
              ...
              <div className="mc-steps-all" style={{ marginTop: 24 }}>
                ...
                {kd.handle && (
                  <div className="mc-section" style={{ paddingTop: 16, borderTop: '1px solid rgba(0,0,0,0.06)' }}>
                    ...
                    {kdBooked ? (
                      <div style={{ padding: 24, background: '#f0faf4', border: '1px solid #b7e5c8' }}>
                        <p style={{ fontWeight: 700, marginBottom: 8 }}>✅ Kit ajouté au panier !</p>
                        <p style={{ fontSize: 14 }}>Finalisez votre commande depuis le panier pour recevoir votre kit.</p>
                      </div>
                    ) : (
                      <form className="ech-form" onSubmit={kdSubmit}>
                        {kdError && <p style={{ color: '#c00', fontSize: 13, marginBottom: 8 }}>{kdError}</p>}
                        <button type="submit" className="ech-form-submit" style={{ background: 'var(--color-noir, #000)' }} disabled={kdSubmitting || !kitProductId}>
                          {kdSubmitting ? 'Ajout en cours…' : `${t('kd_submit')} — ${kitProductPrice} €`}
                        </button>
