...
function ShareModal({ productId, onClose }: ShareModalProps) {
  ...
  const shareUrl =
    typeof window !== 'undefined'
      ? `${window.location.origin}${locale === 'nl' ? `/nl/meubel/${productId}` : locale === 'en' ? `/en/product/${productId}` : `/fr/produit/${productId}`}`
      : `${locale === 'nl' ? `/nl/meubel/${productId}` : locale === 'en' ? `/en/product/${productId}` : `/fr/produit/${productId}`}`
...
function WishlistItemRow({ item, onShare, onRemoved }: WishlistItemRowProps & { onRemoved: (id: number) => void }) {
  ...
  const configurerHref = (() => {
    const dim = (item.rawConfig?.dimension ?? {}) as Record<string, unknown>
    ...
    <div className="wish-item">
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        ...
        {item.config && Object.entries(item.config).map(([k, v]) => (
          <span key={k} style={{ fontSize: '0.9rem', color: '#696761' }}>{v as string}</span>
        ))}
...
export default function WishlistPage() {
  ...
  const [shareProductId, setShareProductId] = useState<number | null>(null)
  const [removedIds, setRemovedIds] = useState<Set<number>>(new Set())

  const handleShare = useCallback((productId: number) => {
  ...
  return (
    <>
          ...
          {loading ? (
            ...
            <div style={{ marginTop: '1.5rem' }}>
              {visibleItems.map(item => (
                <WishlistItemRow key={item.id} item={item} onShare={handleShare} onRemoved={handleRemoved} />
              ))}
            </div>
          )}
        </div>
      ...
      <ShareModal productId={shareProductId} onClose={() => setShareProductId(null)} />
