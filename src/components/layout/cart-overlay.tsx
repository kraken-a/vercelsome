...
function fmt(price: number, locale: string) {
  ...
  return new Intl.NumberFormat(formatLocale, { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(price)
}
  ...
  return (
    <div style={{
      ...
      <div style={{ width: 90, height: 90, background: '#ece9e2', flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
        {item.imageUrl ? (
          <Image src={item.imageUrl} alt={item.name} fill style={{ objectFit: 'cover' }} sizes="90px" />
        ) : (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        ...
        {item.config && (() => {
          ...
          ].filter(Boolean).join(' × ') + ' cm' : null
          const arts = (cfg['aricles'] ?? cfg['articles']) as unknown[] | undefined
          return (
            <>
              {dimStr && <span style={{ fontFamily: "'PP Air Mono', monospace", fontSize: '0.75rem', color: '#0C524E', letterSpacing: '0.03em' }}>{dimStr}</span>}
              {Array.isArray(arts) && arts.map((art, i) => {
                ...
                return (
                  <span key={i} style={{ fontSize: '0.82rem', color: '#696761', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: typeof hex === 'string' ? hex : '#696761', border: '1px solid rgba(0,0,0,0.12)', flexShrink: 0, display: 'inline-block' }} />
                    {label}
                  </span>
                )
              })}
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4rem 0', fontSize: '0.95rem' }}>
                ...
                <span style={{ fontWeight: 500, color: '#1a1a1a' }}>{fmt(cart.subtotal, locale)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4rem 0', fontSize: '0.95rem' }}>
