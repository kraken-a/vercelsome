...
): Promise<Result<CheckoutUrlResponse>> {
  const params: Record<string, string> = {}
  if (tva6 !== undefined) params.tva6 = tva6 ? '1' : '0'
  if (so1Id !== undefined) params.so1_id = String(so1Id)
  if (deliveryPartnerId) params.delivery_partner_id = String(deliveryPartnerId)
  if (invoicePartnerId) params.invoice_partner_id = String(invoicePartnerId)
...
export async function confirmOrder(
  ...
  deliveryPartnerId?: number | null,
  invoicePartnerId?: number | null,
): Promise<Result<ConfirmOrderResponse>> {
  return apiPost<ConfirmOrderResponse>('/cart/confirm-order', {
    order_id: orderId,
