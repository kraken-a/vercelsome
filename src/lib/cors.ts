 ...
 *
 * CSRF NOTE (audit-verified, L2): `verifyCsrfOrigin` falls back to the `Referer`
 * header when `Origin` is absent. This fallback is intentional — some legitimate
 * same-site navigations omit `Origin` but still send `Referer`. When BOTH headers
 * are absent the request is rejected (returns false), which is the safe default.
 * Do not "simplify" by dropping the Referer fallback or by allowing absent-both.
 */
